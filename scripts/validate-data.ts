...
const RewardMomentSchema = z.object({
  when: z.string().min(1),
...
const QuestSchema = z
  .object({
    ...
    // Old model (optional now)
    rewards: z.array(RewardBaseSchema).min(0).optional(),

    // New model (optional)
    rewardMoments: z.array(RewardMomentSchema).min(1).optional(),
  })
  .superRefine((q, ctx) => {
    const hasRewardsArray = Array.isArray(q.rewards) && q.rewards.length >= 0;
    const hasRewardMoments = Array.isArray(q.rewardMoments) && q.rewardMoments.length > 0;

    // Must have at least one of the two models present
    if (!hasRewardsArray && !hasRewardMoments) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Quest must have either "rewards" or "rewardMoments".',
        path: ["rewards"],
      });
    }
  });
